...
def draw_triangle(l, speed):
	...
	while temp < side:
		...
		vel.linear.y = 0.5*kok_value*speed
		vel.linear.z = 0
		...
		temp += sqrt(0.5*0.5*speed*speed + 0.5*kok_value*speed*0.5*kok_value*speed)
		
		pub.publish(vel)
		rate.sleep()
		
		filled += sqrt(0.5*0.5*speed*speed + 0.5*kok_value*speed*0.5*kok_value*speed)
		percentage = filled / (3 * l)
		draw_percent(percentage)
	temp = 0
	...
	while temp < side:
		...
		filled += sqrt(0.5*0.5*speed*speed + 0.5*kok_value*speed*0.5*speed*kok_value)
		percentage = filled / (3 * l)
		draw_percent(percentage)
	temp = 0
	# half side of mid
